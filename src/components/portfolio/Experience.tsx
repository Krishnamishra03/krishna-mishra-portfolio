      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div ref={ref} className="relative mt-16">
          ...
          >
            ...
            {/* base dashed path */}
            <path
              d="M120 0 C 40 200, 200 400, 120 500 S 40 800, 120 1000"
...
function PathCard({ step, index, total }: { step: Milestone; index: number; total: number }) {
  ...
  return (
    <motion.div
      ...
      className="relative grid grid-cols-1 items-center gap-6 pl-12 md:grid-cols-2 md:pl-0"
    >
      {/* Waypoint node */}
      <div className="absolute left-4 top-6 md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2">
        <span className="relative grid place-items-center">
          ...
          />
          <span
            className="relative grid size-5 place-items-center rounded-full border-2 border-background"
            style={{ background: step.accent, boxShadow: `0 0 20px ${step.accent}` }}
          >
            <span className="size-1.5 rounded-full bg-background" />
          </span>
          <span className="mt-2 hidden font-mono text-[9px] uppercase tracking-[0.28em] text-foreground/60 md:block">
            0{index + 1} / 0{total}
          </span>
        </span>
      </div>
